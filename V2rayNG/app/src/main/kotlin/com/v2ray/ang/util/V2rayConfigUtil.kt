        ...
        try {
            ...
            val sniffAllTlsAndHttp =
                settingsStorage?.decodeBool(AppConfig.PREF_SNIFFING_ENABLED, true)
                    ?: true
            v2rayConfig.inbounds[0].sniffing?.disabled = fakedns || sniffAllTlsAndHttp
    ...
    private fun mux(outbound: V2rayConfig.OutboundBean): Boolean {
        try {
            val enableMux = settingsStorage?.decodeBool(AppConfig.PREF_MUX_ENABLED, false)
                ?: true
            val setConcurrency = Utils.parseInt(
                settingsStorage?.decodeString(AppConfig.PREF_MUX_CONCURRENCY),
                AppConfig.MUX_CONCURRENCY.toInt()
            )
